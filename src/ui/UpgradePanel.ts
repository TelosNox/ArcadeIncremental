...
export class UpgradePanel {
  ...
  private render(): void {
    ...
    for (const definition of UPGRADE_DEFINITIONS) {
      const level = state.machine01Upgrades[definition.id];
      const maxLevel = getUpgradeMaxLevel(definition.id);
      const atMax = maxLevel !== undefined && level >= maxLevel;
      const cost = getUpgradeCost(definition.id, level);
      const affordable = !atMax && state.reflexPunkte.gte(cost);
      ...
      buyButton.textContent = atMax ? 'Max. Stufe erreicht' : `Kaufen — ${formatNumber(cost)} Reflex-Punkte`;
      ...
      buyButton.addEventListener('click', () => {
        ...
        this.store.emit({ type: 'machine01UpgradePurchased', upgradeId: definition.id });
      });
